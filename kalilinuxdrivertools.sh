...
fun_driver () {
    ...
    echo -e "\n${info} Instalar build-essential git dkms linux-headers......${reset}\n"
...
fun_install_tools () {
    ...
    echo -e "\n${info} Instalar hcxdumptool hcxtools......${reset}\n"
    sudo apt install -y hcxdumptool hcxtools

    # Instalar dependencias para compilar Pyrit 
    echo -e "\n${info} Instalar libpcap-dev git python2.7-dev libssl-dev zlib1g-dev libpcap-dev......${reset}\n"
...
printf "          \   \_ \`  -+-  \` _/   / \n"
printf "           \`-       -+-       -\` \n"
printf "                    -+- ${reset}${green}Coded by: Jony Rivera\e[0m\n"
printf "\n"
}
...
echo -e "${green}   Update    :  [2024/01/01]${reset}\n"
...
if [[ "$response" = @(y|Y) ]]; then
    ...
    echo -e "\n     ${red}sudo reboot ${green}o ${red}shutdown -r now, ${green}tambien puede hacer ${red}restart manualmente${reset}\n"
